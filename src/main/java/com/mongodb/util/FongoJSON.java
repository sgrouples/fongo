...
public class FongoJSON {
   ...
   * BSONCallback} during parsing. If the callback {@code c} is null, this method is equivalent to {@link FongoJSON#parse(String)
   * parse(String)}.
   *
  ...
  static void string(final StringBuilder a, final String s) {
    ...
    for (int i = 0; i < s.length(); ++i) {
      ...
      if (c == '\\') {
        ...
        final String string = Integer.toHexString(c);
        int size = string.length();
        string.getChars(0, size, pad, 4 - size);
        a.append("\\u" + new String(pad));
      } else {
        a.append(c);
      }
    }
...
class JSONParser {
  ...
  public JSONParser(final String s, final BSONCallback callback) {
    this.s = s;
    _callback = (callback == null) ? new FongoJSONCallback() : callback;
  }
